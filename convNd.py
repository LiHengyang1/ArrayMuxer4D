        ...
        if not self.is_transposed:
            ...
            if self.rank==0:
                ...
                if self.padding_mode == 'zeros':
                    input = F.pad(input.contiguous().view(input.shape[0],input.shape[1],input.shape[2],-1),padSize,'constant',0).view(inputShape)
                else:
                    input = F.pad(input.contiguous().view(input.shape[0],input.shape[1],input.shape[2],-1),padSize,self.padding_mode).view(inputShape)
